  ...
  <div class="dialog">
    <div class="dialog-header">
      ...
      <button type="button" class="btn-copy" title="Copy name to clipboard"
        onclick="const btn=this;const text=btn.closest('.dialog').querySelector('.dialog-title').textContent;const reset=()=>{clearTimeout(btn._copyFeedbackTimeout);btn.classList.remove('copied','copy-error')};const show=(state)=>{reset();btn.classList.add(state);btn._copyFeedbackTimeout=setTimeout(()=>btn.classList.remove('copied','copy-error'),1500)};const writeText=navigator.clipboard?.writeText;if(!writeText){show('copy-error');return;}writeText.call(navigator.clipboard,text).then(()=>show('copied')).catch(()=>show('copy-error'))">
